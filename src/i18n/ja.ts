export const ja = {
  common: {
    description:
      '下の部屋とドアをクリック、またはドラッグすると、条件を満たすさまざまなルートの確率が表示されます。ドアは1回クリックで開いた状態に、2回クリックで開かない状態に、3回クリックで未確定の状態にできます。',
    resetGridButton: 'ルートをリセット',
    copyLinkButton: 'リンクをコピー',
    copyLinkToClipboardSuccess: 'リンクをクリップボードにコピーしました',
    copyLinkToClipboardError: 'リンクのコピーができませんでした',
    possiblePaths_zero: '{{count}}通りの可能なルート',
    possiblePaths_one: '{{count}}通りの可能なルート',
    possiblePaths_two: '{{count}}通りの可能なルート',
    possiblePaths_threeToTen: '{{count}}通りのルート',
    possiblePaths_other: '{{count}}通りのルート',
    viewOnLargerScreen: '可能なルートのリストを表示するためには、より大きな画面で表示',
  },
};
